...
- 当前轮次定位：G01-G02 收口回归与诊断链验证

## 1. 执行结果

| 类别 | 命令 / 依据 | 结果 | 备注 |
|------|-------------|------|------|
| 格式校验 | `cargo fmt --all` | 通过 | G02 代码合入后执行 |
| 工作区回归 | `cargo test --workspace` | 通过 | `aegis_core` 76 项、`aegis_model` 4 项、`aegis_platform` 12 项，共 92 项单元测试；doc tests 全部通过 |
| 诊断模式 | `cargo run -p aegis-agentd -- --diagnose` | 通过 | 诊断包已输出 `communication`、`runtime_signals`、`plugin_status`、WAL、资源、自保护状态 |
...
| 升级与诊断 | `upgrade` 模块单元测试 + `--diagnose` 实际运行 | 升级规划、灰度门控、通信状态、篡改信号与诊断输出通过 |
