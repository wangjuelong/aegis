...
|--------|------------|----------|--------------------|
| 进程快照 | `Win32_Process` | `Get-CimInstance Win32_Process` 周期性快照差分 | `process_id`、`parent_process_id`、`name`、`command_line`、`image_path`、`signature_status`、`signer_subject` |
| 线程快照 | `Win32_Thread` | `Get-CimInstance Win32_Thread` 周期性快照差分 | `thread_id`、`process_id`、`thread_state`、`start_address` |
...
| 内存区域 | 真实虚拟内存区域枚举 | 内置 `query-memory-regions.ps1` 调用 `OpenProcess / VirtualQueryEx / ReadProcessMemory / GetMappedFileName` | `process_id`、`process_name`、`base_address`、`region_size`、`protection`、`memory_type`、`mapped_path`、`sample_base64` |
| 网络连接 | TCP/UDP 连接清单 | `Get-NetTCPConnection` + `Get-NetUDPEndpoint` 周期性快照差分 | `protocol`、`local_address`、`local_port`、`remote_address`、`remote_port`、`state`、`owning_process` |
| DNS | `Microsoft-Windows-DNS-Client/Operational` | 启用后通过 `Get-WinEvent` 增量拉取 | `record_id`、`event_id`、`query_name`、`query_type`、`query_status`、`response_status`、`query_results`、`dns_server_ip`、`client_pid` |
| TLS | System 日志 `Schannel` provider | `Get-WinEvent -FilterHashtable @{ LogName='System'; ProviderName='Schannel' }` 增量拉取 | `record_id`、`event_id`、`caller_process_id`、`caller_process_image_name`、`error_code`、`cert_subject`、`cert_issuer`、`cert_thumbprint` |
| Named Pipe | `\\.\pipe\` 目录 | `Get-ChildItem -Path '\\.\pipe\'` 周期性快照差分 | `pipe_name` |
| 模块 | 进程模块清单 | `Get-Process | $_.Modules` 周期性快照差分 | `process_id`、`process_name`、`module_path`、`signature_status`、`signer_subject` |
...
| 卷挂载 | `Get-Volume` | 周期性快照差分 | `drive_letter`、`path`、`file_system_label` |
...
|----------|----------------|------|----------|
| 进程创建 | `process-start` | `Win32_Process` 快照差分 | `pid`、`ppid`、`name`、`cmdline`、`image`、`signature_status`、`signer` |
| 进程终止 | `process-exit` | `Win32_Process` 快照差分 | `pid`、`ppid`、`name`、`cmdline` |
| 进程创建审计 | `process-audit` | Security `4688` | `record_id`、`process`、`cmdline` |
| 线程创建 | `thread-start` | `Win32_Thread` 快照差分 | `tid`、`pid`、`thread_state`、`start_address` |
| 线程终止 | `thread-exit` | `Win32_Thread` 快照差分 | `tid`、`pid`、`thread_state` |
...
| 私有可执行页 | `memory-private-exec` | 虚拟内存区域枚举 | `pid`、`name`、`region_base`、`region_size`、`protection`、`memory_type`、`mapped_path`、`sha256` |
| RWX / 可写可执行页 | `memory-rwx` | 虚拟内存区域枚举 | 同上 |
| 映像映射异常 | `memory-image-anomaly` | 虚拟内存区域枚举 | 同上 |
| 内存规则命中 | `memory-yara-match` | 区域样本规则匹配 | `pid`、`name`、`region_base`、`region_size`、`protection`、`memory_type`、`mapped_path`、`sha256`、`yara_rule` |
...
| DNS 查询 | `dns-query` | DNS Client Operational | `record_id`、`event_id`、`query_name`、`query_type`、`query_status`、`query_results`、`client_pid` |
| DNS 响应 | `dns-response` | DNS Client Operational | `record_id`、`event_id`、`query_name`、`query_type`、`response_status`、`query_results`、`dns_server_ip`、`client_pid` |
| TLS 证书校验失败 | `tls-cert-validation-failure` | System `Schannel` | `record_id`、`event_id`、`pid`、`process`、`error_code`、`cert_subject`、`cert_issuer`、`cert_thumbprint` |
| TLS 告警 | `tls-alert` | System `Schannel` | 同上 |
...
| Named Pipe 打开 | `pipe-open` | Minifilter 文件事件上的 Pipe 路径识别 | `sequence`、`pid`、`pipe`、`source_operation` |
| Named Pipe 写入 | `pipe-write` | Minifilter 文件事件上的 Pipe 路径识别 | `sequence`、`pid`、`pipe`、`source_operation` |
| Named Pipe 删除 | `pipe-delete` | Minifilter 文件事件上的 Pipe 路径识别 | `sequence`、`pid`、`pipe`、`source_operation` |
| 模块可见 | `module-visible` | 进程模块清单差分 | `process_id`、`process_name`、`module_path`、`signature_status`、`signer` |
| 模块消失 | `module-gone` | 进程模块清单差分 | `process_id`、`process_name`、`module_path`、`signature_status`、`signer` |
...
| VSS 快照创建 | `shadow-create` | `Win32_ShadowCopy` 差分 | `snapshot_id`、`volume_name` |
| VSS 快照删除 | `shadow-delete` | `Win32_ShadowCopy` 差分 | `snapshot_id`、`volume_name` |
...
| 卷挂载新增 | `device-mount-add` | `Get-Volume` 差分 | `drive_letter`、`path`、`label` |
| 卷挂载移除 | `device-mount-remove` | `Get-Volume` 差分 | `drive_letter`、`path`、`label` |
| 卷挂载变化 | `device-mount-change` | `Get-Volume` 差分 | `drive_letter`、`path`、`label` |
