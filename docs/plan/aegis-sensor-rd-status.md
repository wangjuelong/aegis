...
| L04 | Linux 真实 eBPF 资产与特权安装/验证链路 | in_progress | 已入仓 `packaging/linux-ebpf/` 真实 `process/file/network` BPF 资产、`autoattach`/`pinmaps` 运行时模型，以及 `scripts/linux-ebpf-{sync,install,verify,uninstall}.sh`；本地 `bash -n` 与 `cargo test -p aegis-platform` 通过，且在测试机上已验证真实编译、装载、pin/link/map 落盘。为启用 LSM 强制执行已写入 `bpf` LSM 内核参数，但测试机重启后离线，最终强制执行验收待机器恢复后补齐 |
| L05 | Linux TPM sealed-object 主密钥路径 | in_progress | 已在 `aegis-core` 接入 sealed-object 主路径、sealed 优先 / NV fallback 逻辑、`TPM2TOOLS_TCTI` 设备绑定，以及 `scripts/linux-tpm-sealed-verify.sh` 真机验收脚本；本地 `cargo test -p aegis-core linux_tpm -- --nocapture` 通过，测试机 `tpm2-tools` 安装与 create/unseal 真机 roundtrip 待机器恢复后补齐 |
