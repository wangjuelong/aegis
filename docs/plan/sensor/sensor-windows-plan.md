...
- 早先代码审查确认的驱动/保护面缺口已经收口，但围绕“Windows 是否满足 EDR 级数据采集要求”的再审查又识别出 5 个新增缺口；其中 `W22-W24` 已完成，当前还剩 2 个未完成。
...
- 因此仓库侧 Windows 保护面缺口已清零，但 Windows EDR 数据采集面仍保持 `doing`，除正式 AMSI strict-block 验证主机前置条件外，还需继续收口 Device/Pipe/VSS 行为、内存注入/YARA 两个采集缺口；认证采集 `W22`、深度网络遥测 `W23`、进程线程模块细粒度采集 `W24` 已完成。
...
5. 再完成 `W25-W26` 两个 Windows EDR 数据采集缺口
