...
- 早先代码审查确认的驱动/保护面缺口已经收口，但围绕“Windows 是否满足 EDR 级数据采集要求”的再审查又识别出 5 个新增缺口；其中 `W22-W25` 已完成，当前还剩 1 个未完成。
...
- `W16` 已完成主体：`block_hash` / `block_pid` / `block_path` 已切到真实 Minifilter block map、TTL、事件回传与清空链路。
...
- 因此仓库侧 Windows 保护面缺口已清零，但 Windows EDR 数据采集面仍保持 `doing`，除正式 AMSI strict-block 验证主机前置条件外，还需继续收口内存注入/YARA 一个采集缺口；认证采集 `W22`、深度网络遥测 `W23`、进程线程模块细粒度采集 `W24`、Device/Pipe/VSS 行为采集 `W25` 已完成。
...
| W23 | Windows 深度网络遥测闭环 | done | 不允许继续只有连接清单差分；必须补齐 DNS/TLS 元数据 | 已完成 DNS Client Operational、Schannel 增量链与真机验证；详见 `sensor-windows-network-deep-telemetry-plan.md` |
| W24 | Windows 进程线程模块细粒度采集闭环 | done | 不允许继续只有进程快照和模块可见性；必须补齐线程、镜像路径和签名上下文 | 已完成 `ThreadSensor`、镜像路径/签名字段与模块签名上下文；详见 `sensor-windows-process-thread-module-plan.md` |
| W25 | Windows Device / Pipe / VSS 行为采集闭环 | done | 不允许继续只有 visible/gone 资产视图；必须补齐行为事件 | 已完成 Pipe 行为派生、VSS create/delete 与卷挂载差分；详见 `sensor-windows-device-pipe-vss-behavior-plan.md` |
| W26 | Windows 内存注入 / YARA / 映射采集闭环 | todo | 不允许继续把内存侧停留在资源热点；必须补齐私有可执行页、映射异常和 YARA 信号 | 详见 `sensor-windows-memory-injection-plan.md` |
...
- Windows MSI 工程：`done`
- Windows 认证采集闭环：`done`
- Windows 深度网络遥测闭环：`done`
- Windows 进程线程模块细粒度采集闭环：`done`
- Windows Device / Pipe / VSS 行为采集闭环：`done`
- Windows 内存注入 / YARA / 映射采集闭环：`todo`
...
5. 再完成 `W26` 最后一个 Windows EDR 数据采集缺口
