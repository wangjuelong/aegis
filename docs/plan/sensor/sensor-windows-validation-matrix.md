...
- 实际验证时间：`2026-04-20 14:43:43 +08:00`
- 实际验证 ID：`windows-runtime-20260420-144321`
...
| `process_inventory` | pass | `Win32_Process=254`，`tasklist=255` |
| `security_4688` | pass | 可读取 `Security/4688`，最新 `record_id=788750`；临时 `cmd.exe` 探针未稳定命中，作为观察项保留 |
| `network_inventory` | pass | `TCP=193`，`UDP=61` |
| `firewall_block` | pass | 成功创建并清理 `AegisValidation-92dee41c` rule group |
...
| `suspend_kill_response` | pass | 真实执行 `NtSuspendProcess + Stop-Process`，目标 `pid=8160` |
| `quarantine_response` | pass | 真实移动文件到 `C:\ProgramData\Aegis\quarantine\windows-runtime-20260420-144321-quarantine-input.txt` |
| `forensics_response` | pass | 真实生成 `C:\ProgramData\Aegis\forensics\windows-runtime-20260420-144321.zip` |

## 5. 关键产物

- 验收摘要：`C:\ProgramData\Aegis\validation\windows-runtime-20260420-144321\summary.json`
- 隔离产物：`C:\ProgramData\Aegis\quarantine\windows-runtime-20260420-144321-quarantine-input.txt`
- 取证目录：`C:\ProgramData\Aegis\forensics\windows-runtime-20260420-144321`
- 取证压缩包：`C:\ProgramData\Aegis\forensics\windows-runtime-20260420-144321.zip`

## 6. 结论

- `scripts/windows-runtime-verify.sh` 已可从仓库直接复跑 Windows 真机验收。
- `192.168.2.218` 已验证通过当前已完成的 Windows 运行时、网络隔离、响应链、DPAPI 凭据保护与 TPM 观测链。
