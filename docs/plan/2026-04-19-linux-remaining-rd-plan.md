...
- TPM sealed-object 主密钥路径与 PCR policy session
- TPM quote/checkquote attestation baseline
- Linux 容器验证
...
- B06：done。sealed-object 主密钥路径已新增 PCR policy 配置、policy digest 生成与 `session:<ctx>` 解封；测试机 `192.168.1.6` 已通过 `scripts/linux-tpm-policy-verify.sh` 的正反向验收，当前 Linux 可在测试机闭环的 TPM 研发项已全部完成。
...
- Linux 测试机完成 TPM PCR policy session 验证
- 文档中的 Linux 剩余差距不再包含：
  - 真实 `.bpf.o` 资产缺失
  - 安装/装载/附着链路缺失
  - TPM 仅 NV index
