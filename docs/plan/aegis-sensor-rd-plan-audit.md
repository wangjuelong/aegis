...
从“计划覆盖”维度看，终态能力项仍在研发文档中；但从“仓库代码实现”维度看，在 `C01-C03` 完成后，仍存在以下缺失：
...
- 真实热更新/watchdog 仍降为 skeleton 进程
...
该计划将本轮仓库内可闭合工作拆分为 `C01-C04` 四个收口工作包。当前 `C01-C03` 已完成，剩余仓库内 merge gate 为 `C04`；驱动级/硬件级事项继续归档为外部工程，避免把不可在当前仓库中完成的事项伪装成已交付。
