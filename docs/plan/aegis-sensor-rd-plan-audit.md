...
从“计划覆盖”维度看，终态能力项仍在研发文档中；但从“仓库代码实现”维度看，在 `C01-C04` 完成后，剩余缺失已收敛到外部工程边界：

- 三平台真实内核/系统集成未落地
- TPM / Secure Enclave / OS keystore 绑定与正式密钥契约未落地

### 5.2 功能妥协

当前主要妥协已经不再是仓库内骨架化闭环，而是明确保留在外部工程中的系统级依赖：

- 驱动 / System Extension / eBPF/LSM 等系统级集成仍未进入当前仓库
- 硬件根信任与正式设备证书轮换仍未进入当前仓库
...
该计划将本轮仓库内可闭合工作拆分为 `C01-C04` 四个收口工作包。当前 `C01-C04` 已全部完成，仓库内 merge gate 已闭合；驱动级/硬件级事项继续归档为外部工程，避免把不可在当前仓库中完成的事项伪装成已交付。
