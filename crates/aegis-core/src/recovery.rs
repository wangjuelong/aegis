...
impl FilesystemRollbackPlanner {
    pub fn plan(original_path: PathBuf, snapshot_path: PathBuf) -> Result<FilesystemRollbackPlan> {
        Ok(FilesystemRollbackPlan {
            original_path,
            snapshot_sha256: hash_path(&snapshot_path)?,
            snapshot_path,
        })
    }
    ...
    pub fn restore(plan: &FilesystemRollbackPlan) -> Result<()> {
        ...
        fs::copy(&plan.snapshot_path, &plan.original_path)?;
        Ok(())
    }
}
...
impl EvidenceChain {
    pub fn append(&mut self, bundle: &ArtifactBundle) -> Result<EvidenceEntry> {
        ...
        let chain_hash = chain_hash(bundle.artifact_id, &sha256, previous_hash.as_deref());
    ...
    pub fn verify(&self) -> bool {
        ...
        for entry in &self.entries {
            let expected = chain_hash(entry.artifact_id, &entry.sha256, previous_hash.as_deref());
...
fn chain_hash(artifact_id: Uuid, sha256: &str, previous_hash: Option<&str>) -> String {
    hash_bytes(
        format!(
            "{}|{}|{}",
            artifact_id,
